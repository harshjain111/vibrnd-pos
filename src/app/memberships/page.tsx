...
export default async function MembershipsPage() {
  ...
  const [plans, memberships, items] = await Promise.all([
    db.membershipPlan.findMany({
      ...
      include: { benefits: { include: { item: true } }, _count: { select: { memberships: true } } },
                  ...
                  <CardContent className="space-y-1 text-sm">
                    {p.benefits.map((b) => (
                      <div key={b.id} className="flex items-center gap-2">
                        ...
                        <span>{b.name}</span>
                        {b.item && <Badge variant="outline" className="text-[10px]">{b.item.name}</Badge>}
                        <Badge variant="secondary" className="text-[10px]">{b.qtyPerDay}/day · across outlets</Badge>
                      </div>
                    ))}
                    <PlanDialog
                      items={items}
                      initial={{
                        ...
                        benefitName: p.benefits[0]?.name ?? "",
                        benefitItemId: p.benefits[0]?.itemId ?? "",
                        qtyPerDay: p.benefits[0]?.qtyPerDay ?? 1,
                      }}
                    >
